...
extern int create_virtual_touchscreen(void);
			       ...
			       struct goodix_point_t *points)
{
	...
	for (i = 0; i < ts->pdata->max_touch_id; i++) {
		if (touch_num && i == points->id) {
			...
			// slot = slot_conver_mechanism((void *)ts->input_dev, points->id, TOUCH_DOWN_T);
			...
			// slot = slot_conver_mechanism((void *)ts->input_dev, i, TOUCH_UP_T);
...
static void gtp_work_func(struct goodix_ts_data *ts)
{
	...
	if (!ts->pdata->type_a_report) {
		...
		if (ret < 0) {
			dev_err(&ts->client->dev, "virtual_touch_sem request in touchscreen error\n");
		} else {
...
static int gtp_wakeup_sleep(struct goodix_ts_data *ts)
{
	...
	while (retry++ < 10) {
		...
		if (!ret) {
			...
			if (ret < 0) {
				dev_err(&ts->client->dev, "Failed create work thread in resume");
			}
			if (ts->pdata->esd_protect)
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	ts->input_dev->evbit[0] = BIT_MASK(EV_SYN) | BIT_MASK(EV_KEY)
		| BIT_MASK(EV_ABS);
	if (!ts->pdata->type_a_report) {
		input_mt_init_slots(ts->input_dev, 0x16, INPUT_MT_DIRECT);
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret) {
		...
		/*	
			If cannot successfully communicate with the touch-chip, 
			need to treat it as the hardware does not support it,
			but a virtual touch screen must be created to map the upper mouse mode.
		*/
		ret = create_virtual_touchscreen();
		if (ret != 0) {
			dev_err(&client->dev, "can't create virtual touch\n");
		}
...
static int gtp_pm_suspend(struct device *dev)
{
	...
	if (ts) {
		dev_err(&ts->client->dev, "Suspend by i2c pm.");
		gtp_suspend(ts);
	}
...
static int gtp_pm_resume(struct device *dev)
{
	...
	if (ts) {
		dev_err(&ts->client->dev, "Resume by i2c pm.");
		gtp_resume(ts);
	}
